                ...
                if (result.relaypoint_delivery === true) {
                    if (result.addresses) {
                        ...
                        for (var index in relaypoint_addresses) {
                            ...
                            var label = '<label class="label-optional" style="width:90%;cursor:pointer;" for=' + id +'> ' + address.name + '<p class="text-muted">' + details.filter(Boolean).join(', ') + '</p></label>';
                            $delivery_addresses_list.append('<li class="list-group-item li-item" >' + input + label + '</li>');
