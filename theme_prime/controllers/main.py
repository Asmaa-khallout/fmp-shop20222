...
class ThemePrimeWebsiteSale(WebsiteSale):
    ...
    def shop(self, page=0, category=None, search='', min_price=0.0, max_price=0.0, ppg=False, **post):
        ...
        response = super(ThemePrimeWebsiteSale, self).shop(page, category, search, min_price, max_price, ppg, **post)
...
class ThemePrimeMainClass(http.Controller):
    ...
    def _get_products(self, domain=None, fields=[], limit=25, order=None):
        ...
        products = request.env['product.template'].with_context(pricelist=pricelist.id).sudo().search(final_domain, limit=limit, order=order)
    ...
    def _prepare_product_data(self, products, fields, options=None):
        ...
        for res_product, product in zip(result, products):
            ...
            if 'dr_label_id' in fields and product.dr_label_id:
                ...
                res_product['label_template'] = IrUiView._render_template('theme_prime.product_label', values={'label': product.dr_label_id})
            # rating
...
class ThemeWebsite(Website):
    ...
    @http.route('/website/dr_autocomplete', type='json', auth='public', website=True)
    def dr_autocomplete(self, search_type=None, term=None, order=None, limit=5, max_nb_chars=999, options=None):
        """
    ...
    def dr_search(self, term, max_nb_chars, options, **kw):
        ...
        if product_limit:
            results['products'] = self.dr_autocomplete(search_type='products_only', term=term, order='name asc', limit=product_limit, options=options)
