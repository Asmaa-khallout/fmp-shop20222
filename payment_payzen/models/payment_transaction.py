...
class TransactionPayzen(models.Model):
    ...
    def _get_specific_rendering_values(self, processing_values):
        ...
        if self.provider not in ['payzen', 'payzenmulti']:
            return res

        values = self.acquirer_id.payzen_form_generate_values(processing_values)
        ...
        values['payzen_signature'] = self.acquirer_id._payzen_generate_sign(self, values)
        values['api_url'] = self.acquirer_id.payzen_get_form_action_url()
        return values
    ...
    def _get_tx_from_feedback_data(self, provider, data):
        ...
        if provider != 'payzen' and self.provider != 'payzenmulti':
            return tx

        return self._payzen_form_get_tx_from_data(data)
