    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        ...
        if name:
            ...
            if not product_ids and self._context.get('partner_id'):
                customers_ids = self.env['product.customer.info']._search([('partner_id', '=', self._context.get('partner_id')), '|', ('customer_product_code', operator, name), ('customer_product_name', operator, name)], access_rights_uid=name_get_uid)
                suppliers_ids = self.env['product.supplierinfo']._search([('name', '=', self._context.get('partner_id')), '|', ('product_code', operator, name), ('product_name', operator, name)], access_rights_uid=name_get_uid)
                if customers_ids or suppliers_ids:
                    product_ids = self._search(['|', ('product_tmpl_id.customer_ids', 'in', customers_ids), ('product_tmpl_id.seller_ids', 'in', suppliers_ids)], limit=limit, access_rights_uid=name_get_uid)
        else:
...
class SaleOrderLine(models.Model):
    ...
    def get_sale_order_line_multiline_description_sale(self, product):
        ...
        if product.customer_ids:
            customer_id = self.env['product.customer.info'].search([('product_id', '=', product.id), ('partner_id', '=', partner_id.id)], limit=1)
            print(customer_id)
